...
class StateFold_Embeder(nn.Module):

    def __init__(self, model_conf):
        super(StateFold_Embeder, self).__init__()
...
class StateFold_Position_Embeder(nn.Module):
...
class FullScoreNetwork(nn.Module):
    ...
    def __init__(self, model_conf, diffuser):
        ...
        self._model_conf = model_conf
        self.embedding_layer = StateFold_Embeder(model_conf)

        self.diffuser = diffuser
        self.score_model = ipa_pytorch_dynamic.StateFoldIpaScore(model_conf, diffuser)
        self.expand_node = nn.Linear(384, model_conf.node_embed_size)
    ...
    def forward(self, input_feats,drop_ref=False,is_training=True):
        ...
        if self._model_conf.embed.StateFold_embedder:
