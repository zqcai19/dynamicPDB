...
from openfold.utils.script_utils import (load_models_from_command_line,run_model,# parse_fasta,
...
def main(args):
    ...
    for model, output_directory in model_generator:
        ...
        for (tag, tags), seqs in sorted_targets:
            ...
            print('=======>output_name:',output_name,'   ||   ',datetime.datetime.now())
            if output_name == '6sup_A':
                continue
            if os.path.exists(os.path.join(output_directory, output_name)+'.npz'):
            ...
            print(out.keys())
            print(output_name,output_directory)
            print(out['pair'].shape,out['single'].shape)#(38, 38, 128) (38, 384)
            np.savez(os.path.join(output_directory, output_name)+'.npz', single=out['single'], pair=out['pair'])
            print(os.path.join(output_directory, output_name))
            ...
            unrelaxed_file_suffix = ".pdb"
            if args.cif_output:
                unrelaxed_file_suffix = ".cif"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cpus", type=int, default=cpu_count(),
