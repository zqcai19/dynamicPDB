...
class Evaluator:
    def __init__(
        ...
        project_name = 'conformation_sampling'
    ...
    def _load_ckpt(self, conf_overrides):
        ...
        self.exp = train_SFOLD_dynamics.Experiment(conf=self._conf)
    ...
    def create_dataset(self,is_random=False):
        if self._data_conf.is_extrapolation:
            test_dataset = Sfold_data_loader_dynamic.PdbDatasetExtrapolation(
            ...
            test_dataset = Sfold_data_loader_dynamic.PdbDataset(
                data_conf=self._data_conf,
                diffuser=self.exp._diffuser,
                is_training=False,
                is_testing=True,
    ...
    def start_evaluation(self):
        ...
        if self._conf.eval.mode==0:
            ...
            ckpt_eval_metrics,curve_fig,curve_fig_aligned,model_ckpt_update = self.exp.eval_fn(eval_dir,test_loader,self.device,
            ...
            print('='*10, 'Eval TICA')
            self.exp.eval_TICA(eval_dir,test_loader,self.device,noise_scale=self._exp_conf.noise_scale,is_training=False)
        elif self._conf.eval.mode==-2:
            print('='*10, 'Eval Iterative')
            self.exp.eval_iterative(eval_dir,test_loader,self.device,in_diffuser=self.exp._diffuser,exp_name=self._weights_path.split('/')[-3],data_conf=self._data_conf,
                                num_workers=self._exp_conf.num_loader_workers,eval_batch_size=self._eval_conf.eval_batch_size,iterative_time=self._data_conf.frame_time,
...
@hydra.main(version_base=None, config_path="./config", config_name="eval_StateFold")
def run(conf: DictConfig) -> None:
    # Read model checkpoint.
    print('Starting inference')
    start_time = time.time()
    sampler = Evaluator(conf)
    ckpt_eval_metrics,curve_fig,curve_fig_aligned,model_ckpt_update = sampler.start_evaluation()
